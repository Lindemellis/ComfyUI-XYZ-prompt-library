...
把 Mask Editor 的 `mask_0`、`mask_1`… **按顺序**接进 Attach Masks，并且 **不要接** `base` 和
`fill`。接了 `base`，所有索引整体后移 2——不报错、不警告，只是出图不对。
...
|---|---|---|
| 0 | `base` | 覆盖整个画布的全白遮罩 |
| 1 | `fill` | 所有矩形**未**覆盖到的区域 |
| 2… | `mask_0`、`mask_1`… | 每个矩形一张，按列表顺序 |

`base` 和 `fill` 是**给别的节点用的**——区域条件类节点、inpaint 等等。PLv3 用不到它们：它的 base
区域隐含就是全图，fill 由编译器自己算。不管你用不用，它们都固定占据槽位 0 和 1。
