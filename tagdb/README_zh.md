...
- 建议也会匹配标签的**别名**。当某行是靠别名（而非名称本身）命中时，会在该行显示那个命中的别名，让你清楚它为什么出现（例如输入 `indoor` 时，`nude` 通过别名 `indoor_nudity` 命中并显示该别名）。
- **Anima `@artist` 语法**（需开启，*Autocomplete → Anima "@artist" syntax*）：输入 `@name` 时，会按名称 / 别名 / 旧名匹配出**画师**标签（插入为 `@name`），同时也列出名称里真的含有 `@name` 的标签。点击 `@name` 提示词会显示该画师的详情 + 相关标签；若整个 `@name` 本身就是一个真实标签，则改为展示该标签。也兼容权重/包裹写法，点击 `(@wlop:1.1)` 会解析到画师 `wlop`。
- 相关设置（**XYZ Prompt Tools → …**）：
  - **Autocomplete**：总开关、最大建议数（默认 15）、隐藏冷门标签（低于某 post 数则跳过；默认 0 = 全显示）、Anima `@artist` 语法。
  - **Insertion**：下划线→空格、自动逗号、转义括号、全角→半角。
  - **Library**：同时建议你自己的提示词库条目 / 引用。
  - **Related**：在富文本编辑器 / 条目文本视图里点击某标签，查看其详情 + 相关标签（每次查询一个请求；有缓存）。能识别带权重/包裹的写法——点击 `(wlop:1.1)` 会查 `wlop`。
