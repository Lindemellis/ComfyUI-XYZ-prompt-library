...
| `fit` | 会发生什么 |
|---|---|
| **keep** | 图片保持自己的像素尺寸,居中。画布完全不动 —— 比画布大就直接超出去,Krita 的绘画图层允许在画布外持有像素。 |
| **fit** *(默认)* | 图片缩放到画布,**保持长宽比**,居中。多出来的部分保持透明。小了放大,大了缩小。 |
| **grow_canvas** | 图片**比画布大**时,画布放大到图片尺寸,原有内容按**同一个倍率**等比放大(不会变形),图片 1:1 放进去。图片不比画布大时等同于 **keep** —— 没有可放大的理由。 |

**grow_canvas 就是你的放大流程**:2 倍生成,推回去,然后在新尺寸上继续手改和重绘。草稿图层会在这个
过程中变糊,但没关系 —— 走到放大这一步时草稿已经功成身退了。想回到生成分辨率,用 Fetch Image 的
`by_height`。

三个模式都遵守两条规则:

- **画布只增不减。** 这里没有任何东西能让你的文档变小,任何一个方向都不会。这也是为什么
  `grow_canvas` 在每个轴上取图片和画布中**较大**的那个 —— 否则一张比画布宽但比画布矮的图,
  会把你文档的下半截裁掉。
- **没填满画布的部分居中** —— 小图,或者保持长宽比留下的空边。

`grow_canvas` 放大原有内容时,用的是能让它**完整装进**新画布的那个倍率,所以不会有内容被挤出边缘。
512×512 画布配 1024×768 的图就是 ×1.5:原内容变成 768×768,居中放在新的 1024×768 画布里,
图片正好填满。
